...
class FactorizedSpectralConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2, modes3, n_layers=1, bias=True, scale='auto',
        ...
        if factorization is None:
            self.weight = nn.Parameter(scale * torch.randn(4*n_layers, in_channels, out_channels, self.modes1, self.modes2, self.modes3,
                                                            dtype=torch.cfloat))
            self._get_weight = self._get_weight_dense
        else:
            self.weight = tltorch.FactorizedTensor.new((4*n_layers, in_channels, out_channels, self.modes1, self.modes2, self.modes3),
    ...
    def forward(self, x, indices=0):
        with torch.autocast(device_type='cuda', enabled=False):
            ...
            x_fft = torch.fft.rfftn(x.float(), norm=self.fft_norm, dim=[2, 3, 4])
            ...
            out_fft[:, :, :self.modes1, :self.modes2, :self.modes3] = contract_3D(x_fft[:, :, :self.modes1, :self.modes2, :self.modes3], self._get_weight(indices, 0))
            out_fft[:, :, -self.modes1:, :self.modes2, :self.modes3] = contract_3D(x_fft[:, :, -self.modes1:, :self.modes2, :self.modes3], self._get_weight(indices, 1))
            out_fft[:, :, :self.modes1:, -self.modes2:, :self.modes3] = contract_3D(x_fft[:, :, :self.modes1:, -self.modes2:, :self.modes3], self._get_weight(indices, 2))
            out_fft[:, :, -self.modes1:, :self.modes2, :self.modes3] = contract_3D(x_fft[:, :, -self.modes1:, :self.modes2, :self.modes3], self._get_weight(indices, 1))
            out_fft[:, :, -self.modes1:, -self.modes2:, :self.modes3] = contract_3D(x_fft[:, :, -self.modes1:, -self.modes2:, :self.modes3], self._get_weight(indices, 3))
...
class FactorizedSpectralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, modes2, n_layers=1, bias=True, scale='auto',
                 fft_norm='backward',
                 rank=0.5, factorization='cp', fixed_rank_modes=None, decomposition_kwargs=dict(), **kwargs):
        super().__init__()
        ...
        if factorization is None:
            self.weight = nn.Parameter(scale * torch.randn(2*n_layers, in_channels, out_channels, self.modes1, self.modes2,
                                                            dtype=torch.cfloat))
            self._get_weight = self._get_weight_dense
        else:
            self.weight = tltorch.FactorizedTensor.new((2*n_layers, in_channels, out_channels, self.modes1, self.modes2),
                                                        rank=self.rank, factorization=factorization, 
                                                        dtype=torch.cfloat, fixed_rank_modes=fixed_rank_modes,
                                                        **decomposition_kwargs)
            self.weight = self.weight.normal_(0, scale)
    ...
    def forward(self, x, indices=0, super_res=1):
        with torch.autocast(device_type='cuda', enabled=False):
            ...
            out_fft[:, :, :self.modes1, :self.modes2:super_res] = contract_2D(x[:, :, :self.modes1, :self.modes2], self._get_weight(indices, 0))
            # Lower block    
            out_fft[:, :, -self.modes1:, :self.modes2:super_res] = contract_2D(x[:, :, -self.modes1:, :self.modes2], self._get_weight(indices, 1))
